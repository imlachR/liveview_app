defmodule Trivium.Accounts do
  ...
  alias Trivium.Repo
  alias Trivium.Accounts.{User, UserToken, UserNotifier, Avatar}
  ...
  # used in subscriptions policy
  # used in classroom show
  def get_user!(id) do
    User
    |> Repo.get!(id)
    |> Repo.preload(:avatar)
  ...
  def user_progresses(user) do
    Repo.all Ecto.assoc(user, :progresses)
  end

  # used at activities subscribed
  def user_policy(user, classroom_id) do
    Repo.one Ecto.assoc(user, [:subscriptions])
    |> where([s], s.classroom_id == ^classroom_id)
    |> select([s], s.policy)
  end

  # used at activities subscribed
  def access_policy_for(user, classroom_id) do
    Repo.all Ecto.assoc(user, [:subscriptions])
    |> where([s], s.classroom_id == ^classroom_id)
    |> where([s], s.accepted == true)
  end

  # used at classroom show
  def access_policy_for_staff(user, classroom_id) do
    Repo.all Ecto.assoc(user, [:subscriptions])
    |> where([s], s.classroom_id == ^classroom_id)
    |> where([s], s.accepted == true)
    |> where([s], s.policy == "teacher")
  end
  ...
  def update_user_language(%User{} = user, attrs) do
    user
    |> User.lang_changeset(attrs)
    |> Repo.update()
  end
  ...
  """
  def create_avatar(avatar, attrs \\ %{}, after_save) do
    avatar
  ...
  defp after_save({:ok, avatar}, func) do
    {:ok, _avatar} = func.(avatar)
  end
